...
conn = sqlite3.connect("Data/Databases/Master.sqlite3")
cur = conn.cursor()
...
def check_new_file():
    ...
    for i in cur_dict:
        for j in cur_dict[i]:
            print(i, j)
            conn_current = sqlite3.connect("Data/Databases/"+i+".sqlite3")
            ...
            for chunk in pd.read_csv("Data/"+i+"/"+i+"-"+j+".csv",dtype=dtypes,converters=converters, chunksize=50000):
                ...
                chunk.to_sql("Data_"+i, conn_current, if_exists="append", index=False)
                print(i,":", j, ":", t, "done")
                t += 1
            sl_no += 1
            print(sl_no, i+"_"+j)
        ...
        conn.commit()
        conn_current.commit()
        cur_current.execute("create index if not exists prod_index_"+i+" on Data_"+i+" (PRODUCT_DESCRIPTION)")
        cur_current.execute("create index if not exists imp_index_"+i+" on Data_"+i+" (IMPORTER_NAME)")
        cur_current.execute("create index if not exists sup_index_"+i+" on Data_"+i+" (SUPPLIER_NAME)")
        conn.commit()
        conn_current.commit()
